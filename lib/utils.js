...
var utils = {
    ...
    makePostUrl: function (key, config) {
        ...
        if (key.match(/(.+)\//)){
            ...
            if (config && config.postUrlFormat) {
                ...
                return {
                    filePath: utils.completePath(url + ".html"),
                    url: utils.completeUrl(url + ".html")
                }
            }
        }
     ...
     * @returns {*}
     */
    prepareFrontVars: function (posts, data, config) {
